    ...
    const loadTasks = async () => {
        try {
            ...
            if (res.ok) {
                ...
                console.error("Failed to fetch editor tasks")
            }
        } catch (e) {
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-300">
            ...
            <div className="bg-gradient-to-br from-slate-900 via-emerald-950 to-teal-950 p-8 rounded-3xl text-white shadow-xl flex flex-col md:flex-row md:items-center md:justify-between gap-6 border border-emerald-500/20 relative overflow-hidden group hover:shadow-emerald-950/20 hover:shadow-2xl transition-all duration-300">
                ...
                <div className="space-y-2 relative z-10">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs uppercase font-extrabold tracking-widest text-emerald-400">Editor Workbench</span>
                    </div>
                    <h2 className="text-2xl md:text-3xl font-extrabold">Creative Suite Dashboard</h2>
                    <p className="text-slate-450 text-sm max-w-xl leading-relaxed">
                        Monitor your task queue, track rendering milestones, and manage uploads for finalized corporate campaigns.
                    </p>
                </div>
                <div className="shrink-0 flex items-center gap-4 bg-white/5 p-4 rounded-xl border border-white/10 relative z-10 hover:bg-white/10 transition-colors">
                    ...
                    <div>
                        ...
                        <p className="text-sm font-extrabold text-white">{completedTasks} / {totalTasks} Tasks Done</p>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-5">
                {[
                    { label: 'Pending Queue', value: toDoTasks, gradient: 'from-gray-50 to-slate-100/50 text-slate-700 border-slate-200/60 hover:border-slate-350', icon: CircleDot, iconBg: 'bg-slate-100 text-slate-500' },
                    { label: 'In Production', value: inProgressTasks, gradient: 'from-blue-50/50 to-indigo-50/20 text-blue-700 border-blue-100 hover:border-blue-200', icon: Clock, iconBg: 'bg-blue-50 text-blue-500' },
                    { label: 'Awaiting Review', value: reviewTasks, gradient: 'from-amber-50/50 to-yellow-50/20 text-amber-700 border-amber-100 hover:border-amber-200', icon: AlertCircle, iconBg: 'bg-amber-50 text-amber-500' },
                    { label: 'Assets Completed', value: completedTasks, gradient: 'from-emerald-50/50 to-teal-50/20 text-emerald-700 border-emerald-100 hover:border-emerald-200', icon: CheckCircle2, iconBg: 'bg-emerald-50 text-emerald-500' },
                ].map((kpi, idx) => (
                    <div 
                        key={idx} 
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="bg-white/80 backdrop-blur-md p-6 rounded-2xl border border-slate-200/60 shadow-sm flex flex-col justify-between hover:shadow-md transition-all duration-300">
                    <div>
                        <div className="flex justify-between items-center mb-4">
                            <h3 className="font-bold text-gray-900">Task Completion Track</h3>
                            <span className="text-xs bg-emerald-50 text-emerald-600 font-extrabold px-2.5 py-1 rounded-full">{progressPercent}% Completed</span>
                        </div>
                        ...
                        <div className="space-y-4">
                            {[
                                { status: 'To Do', count: toDoTasks, total: totalTasks, color: 'bg-gray-400' },
                                { status: 'In Progress', count: inProgressTasks, total: totalTasks, color: 'bg-blue-500' },
                                { status: 'In Review', count: reviewTasks, total: totalTasks, color: 'bg-amber-500' },
                                { status: 'Completed', count: completedTasks, total: totalTasks, color: 'bg-emerald-500' }
                            ].map((row, rIdx) => {
                    ...
                    <div className="mt-8 pt-4 border-t border-gray-150">
                        <Link href="/editor/videos" className="text-xs font-extrabold text-emerald-600 hover:text-emerald-700 flex items-center justify-between group">
                            <span>Submit Finished Video Asset</span>
                ...
                <div className="lg:col-span-2 bg-white/80 backdrop-blur-md p-6 rounded-2xl border border-slate-200/60 shadow-sm hover:shadow-md transition-all duration-300">
                    <div className="flex justify-between items-center mb-6">
                        <div>
                            <h3 className="font-bold text-gray-900">Immediate Action Items</h3>
                            <p className="text-xs text-gray-500 mt-0.5">High priority tasks scheduled for the next 7 days.</p>
                        </div>
                        <Link href="/editor/schedules" className="text-xs font-bold text-emerald-600 hover:underline">
                            View All Queue
                        </Link>
                    </div>
                    ...
                    {loading ? (
                        ...
                        <div className="h-64 flex flex-col items-center justify-center text-center border-2 border-dashed border-gray-200 rounded-xl">
                            ...
                            <h4 className="font-bold text-gray-900 text-sm">Task Board Empty</h4>
                            <p className="text-xs text-gray-500 mt-1 max-w-xs">No creative tasks or schedules have been loaded for your account yet.</p>
                            <Link href="/editor/schedules" className="mt-4 px-4 py-2 bg-emerald-500 text-slate-950 font-bold rounded-lg text-xs hover:bg-emerald-600 transition-colors">
                                Add First Task
                            </Link>
                        </div>
                    ) : (
                            ...
                            {tasks.filter(t => t.status !== 'Completed' && t.status !== 'Published').slice(0, 4).map(task => (
                                ...
                                >
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className={`px-2 py-0.5 rounded text-[10px] font-extrabold ${getStatusBadgeStyle(task.status)}`}>
                                            {task.status}
                                        </span>
                                    </div>
                                </div>
                ...
                <div className="p-5 border-b border-gray-100 flex items-center justify-between bg-gray-55/10">
                    <div>
                        <h3 className="font-extrabold text-gray-950 text-lg flex items-center gap-2">
                            ...
                            Creative Task Calendar
                        </h3>
                        <p className="text-xs text-gray-500 mt-0.5">Monthly workflow and deadline plotting.</p>
                    </div>
                    ...
                    <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
                        ...
                        <div className="p-5 space-y-4 text-sm">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Due Date & Time</h4>
                                    <p className="text-gray-900 font-semibold">{selectedTask.dueDate} • {selectedTask.time}</p>
                                </div>
                                <div>
                                    <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Asset Type</h4>
                            ...
                            {selectedTask.duration && (
                                <div>
                                    <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Estimated Duration</h4>
                            ...
                            {selectedTask.videoUrl && (
                                <div>
                                    <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Edited Video URL</h4>
                                    <a
                                    ...
                                    >
                                        ...
                                        <span>View Submitted Video Asset</span>
                                    </a>
                                </div>
                            )}
                            ...
                            {selectedTask.notes && (
                                <div>
                                    <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Notes & Guidelines</h4>
                                    <p className="text-gray-600 bg-gray-50 p-2.5 rounded-lg text-xs leading-relaxed border border-gray-150">{selectedTask.notes}</p>
                                </div>
                            )}

                            <div>
                                <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Update Progress Status</h4>
                                <div className="grid grid-cols-3 gap-2">
                                    {['To Do', 'In Progress', 'In Review'].map(st => (
                                        <button
                                            key={st}
                                            disabled={updatingStatusId !== null}
                                            onClick={() => handleStatusChange(selectedTask._id, st)}
                                            className={`py-2 text-xs font-bold rounded-lg border transition-all ${selectedTask.status === st
                                ...
                                <div className="mt-2 flex gap-2">
                                    ...
                                    >
                                        Upload Finalized Asset & Complete Task
                                    </Link>
                                </div>
                            </div>
                        </div>
