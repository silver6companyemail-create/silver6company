...
export default function EditorLayout({ children }: { children: React.ReactNode }) {
  ...
  const loadUserInfo = async () => {
    try {
      ...
      if (userInfoStr) {
        ...
        setUserName(userInfo.name || 'Editor')
  ...
  const getPageTitle = (path: string) => {
    if (path === '/editor') return 'Editor Dashboard'
    if (path.startsWith('/editor/growth')) return 'Growth & Productivity Tracker'
    if (path.startsWith('/editor/schedules')) return 'Task Schedules'
    if (path.startsWith('/editor/videos')) return 'Edited Video Uploads'
    if (path.startsWith('/editor/settings')) return 'Account Settings'
    return 'Creative Overview'
  }
    ...
    <div className="min-h-screen bg-gray-50 flex">
      ...
      >
        ...
        <div className="h-16 flex items-center justify-between px-6 border-b border-slate-100 bg-white/40">
          <Link href="/editor" className="flex items-center gap-2 text-xl font-bold text-slate-900">
            ...
            <span className="bg-gradient-to-r from-slate-900 to-slate-700 bg-clip-text text-transparent font-extrabold tracking-tight">Editor Hub</span>
          </Link>
          <button
        ...
        <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
          {[
            ...
            { href: '/editor/growth', label: 'Growth Tracking', icon: TrendingUp },
            { href: '/editor/schedules', label: 'Task Schedules', icon: Calendar },
            { href: '/editor/videos', label: 'Video Uploads', icon: Video },
            { href: '/editor/settings', label: 'Profile Settings', icon: Settings },
          ].map(item => {
              ...
              {isNotificationsOpen && (
                <div className="absolute right-0 mt-2.5 w-80 bg-white rounded-2xl border border-gray-150 shadow-xl py-2 z-50 overflow-hidden animate-in fade-in slide-in-from-top-3 duration-200">
                  <div className="px-4 py-2 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                    ...
                    <span className="bg-emerald-50 text-emerald-600 text-[10px] font-bold px-2 py-0.5 rounded-full">Editor Alerts</span>
                  </div>
                  ...
                  <div className="p-2 border-t border-gray-100 bg-gray-50/30 text-center">
                    <Link href="/editor/schedules" onClick={() => setIsNotificationsOpen(false)} className="text-[10px] font-bold text-emerald-600 hover:underline">
